...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for p in self.legalPositions:
      ...
      z = emissionModel[trueDistance]
      q = 1/float(len(self.legalPositions))
      e = 0
      for x in emissionModel:
        e += emissionModel[x] * q
      if z > 0: allPossible[p] = z * self.beliefs[p] / e
  ...
  def elapseTime(self, gameState):
    ...
    for p in self.beliefs:       # p is old position (at t-1)
        if (self.beliefs[p] != 0):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            
            for x in newPosDist:
                allPossible[x] += newPosDist[x] * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if (noisyDistance == None):
        for i in range (0, self.numParticles):
            particles[i] = self.getJailPosition()
    else:
        hasNonZero = False
        ctr = util.Counter() # some day, rename this to weightCOunter or similarzzzz
        
        for particle in particles:
            dist = util.manhattanDistance(particle, gameState.getPacmanPosition())
            ctr[particle] = ctr[particle] + emissionModel[dist] 
            hasNonZero = hasNonZero or ctr[particle] != 0
            # now ctr has keys which are positions, and values which are sums of weights for that position
        ...
        for i in range (0, self.numParticles):
            self.particles[i] = util.sample(ctr)
  ...
  def getBeliefDistribution(self):
    ...
    for particle in particles:
        beliefs[particle] = particles.count(particle) / float(self.numParticles)
  ...
  def initializeParticles(self):
    ...
    for i in range (0, self.numParticles):
        ...
        for i in range (0, self.numGhosts):
            lst.append(random.choice(self.legalPositions))
            
        tpl = tuple(lst)
        self.particles.append(tpl)
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
        ...
        for i in range (0, self.numGhosts):
            newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions),
                                                   i, self.ghostAgents[i])
            newGhostPositions.append(util.sample(newPosDist))
        
        newParticle = tuple(newGhostPositions)
